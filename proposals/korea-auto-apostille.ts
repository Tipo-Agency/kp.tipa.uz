...
export const koreaAutoApostille: ProposalData = {
  ...
  hero: {
    ...
    subtitle:
      "Направление 1: импорт фильтров, колодок, свечей и расходников для корейских авто напрямую из Кореи; продажи через Instagram, Uzum и опт через имидж в соцсетях. Направление 2: услуги апостиля и легализации документов «под ключ» (в т.ч. по сканам) для тех, кто в Корее или планирует поездку.",
  },
  services: [
    {
  ...
  servicesDescription:
    "Два направления нельзя смешивать в одной ленте без стратегии: для автозапчастей — товар, логистика, совместимость; для апостиля — юридическая ясность и эмпатия. Ниже — ваши кадры в матрице, позиционировании и сетке визуала. Таргет и блогеры — с отдельным бюджетом.",
  growthTool: {
  ...
  positioning: {
    ...
    description:
      "Позиционируем импорт авторасходников как предсказуемый канал с понятными сроками и честным товаром. Апостиль — как спокойное сопровождение: не обещаем невозможного, выстраиваем доверие через процесс.",
